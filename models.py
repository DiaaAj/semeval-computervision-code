...
class EncoderImageCNN(nn.Module):
    def __init__(self, cfg):
        ...
        if cfg['image-model']['name'] == 'resnet50':
            ...
            modules = list(cnn.children())[:-2]
        elif cfg['image-model']['name'] == 'vgg19':
            cnn = models.vgg19(pretrained=True)
            modules = list(cnn.children())[:-2]
        elif cfg['image-model']['name'] == 'inceptionV3':
            cnn = models.inception_v3(pretrained=True)
        elif cfg['image-model']['name'] == 'densenet161':
            cnn = models.densenet161(pretrained=True)
            modules = list(cnn.children())[:-1]

        # if(cfg['image-model']['name'] == 'efficientnet-b7'):
        #     self.spatial_feats_dim = cnn._fc.in_features
        #     self.cnn = cnn
        #     self.cnn._fc = torch.nn.Sequential( torch.nn.Linear(self.spatial_feats_dim, 512)) 
        # else:
        #self.spatial_feats_dim = cnn._fc.in_features
