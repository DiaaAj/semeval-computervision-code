...
class SemEvalDataset(Dataset):
    def __init__(self, cfg, split='train', val_fold=0, transforms=None):
        ...
        if task == 3:
            if(self.split_name == 'training' and cfg['training']['use_data_aug']):
                label_file = os.path.join(self.data_root, '{}_set_task3'.format(self.split_name), 'task3DataAugmented.txt')    
            else:
