...
def main():
    ...
    cv_models = ['resnet50', 'resnet101', 'vgg19', 'densenet161']

    for cv_model in cv_models:
        print(f"------ Experimenting with {cv_model} ------")
        config['image-model']['name'] = cv_model
...
def train(opt, config, val_fold=0):
    ...
    if(config['image-model']['name'] in ('resnet50','resnet101')):
        config['image-model']['img-size'] = 256
        config['image-model']['feat-dim'] = 2048
        
    if(config['image-model']['name'] == 'vgg19'):
        config['image-model']['img-size'] = 224
        config['image-model']['feat-dim'] = 512
        
    if(config['image-model']['name'] == 'densenet161'):
        config['image-model']['img-size'] = 256
        config['image-model']['feat-dim'] = 2208
        
    # Load data loaders
    test_transforms = T.Compose([T.Resize(config['image-model']['img-size']),
                    T.CenterCrop(config['image-model']['img-size']),
                    T.ToTensor(),
                    T.Normalize(mean=[0.485, 0.456, 0.406],
                                std=[0.229, 0.224, 0.225])])
    train_transforms = T.Compose([T.Resize(config['image-model']['img-size']),
                    T.RandomCrop(config['image-model']['img-size']),
                    T.ToTensor(),
                    T.Normalize(mean=[0.485, 0.456, 0.406],
                                std=[0.229, 0.224, 0.225])])
    ...
    test_dataset = SemEvalDataset(config, split='test', transforms=test_transforms, val_fold=val_fold)
